...
export default function Home() {
  ...
  }, [loaded, tasks, projects, entities, team, clients, goals, menuOrder, campaigns]);
  ...
  function deleteEntity(id: number) {
    if (campaigns.some(c => c.entityIds.includes(id))) { setNotice("Entita je priradená k projektu."); return; }
  ...
  function deleteProject(project: Project) {
    const hasLinkedWork = campaigns.some(c => c.departmentId === project.id) || tasks.some((task) => task.projectId === project.id) || goals.some((goal) => goal.project === project.name);
  ...
  function importData(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        const restored = parsed?.format === "ai-planner-supabase-import-v1" ? parsed.snapshot : parsed;
        const input = Array.isArray(restored) ? { tasks: restored, menuOrder, projects, entities, users: team, clients, goals } : restored;
    ...
    <main className="appShell">
      <aside className="sidebar">
        ...
        <section className="projectList">
          <button aria-expanded={projectsOpen} aria-controls="departmentFilters" onClick={() => setProjectsOpen(!projectsOpen)}><Icon name={projectsOpen ? "down" : "right"} /><span>Oddelenia</span></button>
        ...
        <section className="projectList">
          <button aria-expanded={entitiesOpen} aria-controls="entityFilters" onClick={() => setEntitiesOpen(!entitiesOpen)}><Icon name={entitiesOpen ? "down" : "right"} /><span>Entity</span></button>
      ...
      <section className="content">
        <header className="header">
          ...
          <div className="headerActions"><button className="ghost" title="Lokálna konfigurácia vlastníka; nejde o prihlásenie" onClick={() => setAdminConfig(!adminConfig)}><Icon name="settings" /><span>{adminConfig ? "Zavrieť správu" : "Správa vlastníka"}</span></button>
            ...
            }}><Icon name="upload" /><span>Pripraviť prenos</span></button>
            <button className="ghost" onClick={() => importRef.current?.click()}><Icon name="upload" /><span>Import</span></button>
            <input ref={importRef} className="hiddenInput" type="file" accept="application/json" onChange={importData} />
            <button onClick={openNewTask}><Icon name="plus" /><span>Nova uloha</span></button>
          </div>
        </header>

        <p className="storageStatus" role="status">{remoteStatus()}</p>
        {storageError ? <p className="notice" role="alert">{storageError}</p> : null}
        {notice ? <div className="notice" role="status">{notice} <button className="ghost" onClick={() => setNotice("")}><Icon name="close" /><span>Zavriet</span></button></div> : null}
        ...
        {activeScreen === "Pracovna plocha" ? (
          ...
          <section className="toolbar">
            ...
            <div className="viewSwitch" aria-label="Prepinanie zobrazenia">
              <button className={view === "Tabulka" ? "selected" : ""} onClick={() => setView("Tabulka")}><Icon name="table" /><span>Tabulka</span></button>
              <button className={view === "Kanban" ? "selected" : ""} onClick={() => setView("Kanban")}><Icon name="kanban" /><span>Kanban</span></button>
              <button className={view === "Tyžden" ? "selected" : ""} onClick={() => setView("Tyžden")}><Icon name="calendar" /><span>Tyzden</span></button>
            </div>
          </section>

          <section className="filterSummary toolbar compactFilters">
            ...
            {(["Vsetko", "Dnes", "Vysoka", "Moje", "Hotovo"] as QuickFilter[]).map((filter) => (
              <button key={filter} className={quickFilter === filter ? "selected" : ""} onClick={() => setQuickFilter(filter)}>{filter === "Vsetko" ? "Všetko" : filter === "Vysoka" ? "Vysoká" : filter}</button>
            ))}

            <span>Zobrazene: {visibleTasks.length} z {tasks.length} uloh</span>
            <button className="ghost" onClick={clearFilters}><Icon name="filter" /><span>Vymazať filtre</span></button>
          </section>
          ...
          <section className="board">
            ...
            <div className="tableHeader"><span>Úloha</span><span>Oddelenie / projekt</span><span>Entita</span><span>Osoby</span><span>Status</span><span>Priorita</span><span>Dátum dokončenia</span><span>Akcie</span></div>
            {tableTasks.map((task) => (
              <article className="taskRow" key={task.id}>
                ...
                <div className="rowActions"><button className="ghost iconButton" title="Upraviť" aria-label={`Upraviť ${task.name}`} onClick={() => openEditTask(task)}><Icon name="edit" /></button><button className="ghost iconButton" title="Duplikovať" aria-label={`Duplikovať ${task.name}`} onClick={() => duplicateTask(task)}><Icon name="copy" /></button><button className="danger iconButton" title="Zmazať" aria-label={`Zmazať ${task.name}`} onClick={() => deleteTask(task.id)}><Icon name="trash" /></button></div>
              </article>
            ))}
              ...
              {inboxTasks.map((task) => (
                <article className="inboxItem" key={task.id}>
                  ...
                  <div className="triageActions">
                    ...
                    <button className="danger" onClick={() => deleteTask(task.id)}><Icon name="trash" /><span>Zmazat</span></button>
                  </div>
                </article>
              ))}
            ...
            <div className="recordList" role="table" aria-label="Entity">
              ...
              {entities.map(entity => <div className="entityRow" role="row" key={entity.id}>
                ...
                <div className="recordActions" role="cell">
                  <button className="ghost iconButton" title="Zobraziť úlohy" aria-label={`Úlohy entity ${entity.name}`} onClick={() => chooseEntity(entity.id)}><Icon name="tasks" /></button>
                  <button className="ghost iconButton" title="Upraviť entitu" aria-label={`Upraviť entitu ${entity.name}`} onClick={() => { setEntityDraft(entity); setEditingEntity(true); }}><Icon name="edit" /></button>
                  <button className="danger iconButton" title="Zmazať entitu" aria-label={`Zmazať entitu ${entity.name}`} onClick={() => deleteEntity(entity.id)}><Icon name="trash" /></button>
                </div>
              </div>)}
            </div>
          </section>
        ...
        {activeScreen === "Klienti" && adminConfig ? (
          <section className="clientManager">
            ...
            <div className="clientList">
              ...
              {clients.map(client => <article className="clientRow" key={client.id}>
                ...
                <button className="ghost" onClick={() => { setClientDraft(client); setEditingClient(true); }}><Icon name="edit" /><span>Upravit</span></button>
                <button className="danger" onClick={() => deleteClient(client)}><Icon name="trash" /><span>Zmazat</span></button>
              </article>)}
            </div>
          </section>
        ) : null}
          ...
          <section className="projectManager">
            ...
            <section className="screenGrid">
              {projectHealth.map((project) => {
                const canDelete = !campaigns.some(c => c.departmentId === project.id) && project.tasks.length === 0 && !goals.some((goal) => goal.project === project.name);
                return (
                  <article className="projectSummary" key={project.id}>
                    ...
                    <div className="projectActions">
                      <button className="ghost" onClick={() => chooseProject(project.id)}><Icon name="tasks" /><span>Otvorit ulohy</span></button>
                      <button className="ghost" onClick={() => editProject(project)}><Icon name="edit" /><span>Upravit</span></button>
                      <button className="danger" disabled={!canDelete} title={canDelete ? "Zmazať oddelenie" : "Oddelenie má priradené úlohy alebo historické ciele"} onClick={() => deleteProject(project)}><Icon name="trash" /><span>Zmazat</span></button>
                    </div>
                  </article>
                );
              })}
      ...
      {isFormOpen ? (
        <TaskDialog onClose={() => setIsFormOpen(false)}>
          <form className="modal" onSubmit={saveTask}>
            ...
            <button type="button" className="ghost" onClick={addDraftSlot}><Icon name="plus" /><span>Pridat casovy blok</span></button>
      ...
      {selectedTask ? (
        <aside className="detailPanel">
          <button className="ghost" onClick={() => setSelectedTask(null)}><Icon name="close" /><span>Zavriet</span></button>
          ...
          <button className="ghost wide" onClick={() => openEditTask(selectedTask)}><Icon name="edit" /><span>Upravit ulohu</span></button>
          <button className="danger wide" onClick={() => deleteTask(selectedTask.id)}><Icon name="trash" /><span>Zmazat ulohu</span></button>
        </aside>
      ) : null}
