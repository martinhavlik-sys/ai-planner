...
test('v5 preserves legacy departments, task identities and slots while adding only approved unassigned catalog projects', () => {
 const data = legacy(); const departmentId = data.tasks[0].projectId;
 assert.equal(data.schemaVersion, 6); assert.equal(data.campaigns.length, 22);
...
test('annual projects and multi-entity links roundtrip independently of departments', () => {
 ...
 const canonical = normalizeWorkspace(data); assert.deepEqual(normalizeWorkspace(canonical), canonical); assert.deepEqual(canonical.tasks, data.tasks);
 data.campaigns[0].archived = true;
 assert.equal(normalizeWorkspace(data).tasks[0].campaignId, 2026);
});
test('independent project classification accepts unmatched scope but rejects dangling links', () => {
 ...
 assert.doesNotThrow(() => normalizeWorkspace(data));
 data.tasks[0].campaignId = 999; assert.throws(() => normalizeWorkspace(data), /Projekt/); data.tasks[0].campaignId = 10;
